            ...
            <head>
                ...
                <style>
                  :root {
                    color-scheme: dark;
                    --bg: ${design.bg ?? "#13151a"};
                    --rail: ${design.rail ?? "#1e1f24"};
                    --content-bg: ${design.contentBg ?? "#2b2d31"};
                    --panel: ${design.panel ?? "#313338"};
                    --panel-2: ${design.panel2 ?? "#3a3d43"};
                    --text: ${design.text ?? "#eef2ff"};
                    --muted: ${design.muted ?? "#b5bac1"};
                    --primary: ${design.primary ?? "#5865f2"};
                    --success: ${design.success ?? "#20c997"};
                    --warning: ${design.warning ?? "#f4c95d"};
                    --danger: ${design.danger ?? "#ff6b6b"};
                    --info: ${design.info ?? "#4dabf7"};
                  ...
                  * { box-sizing: border-box; }
                  ...
                  .sidebar { width: 76px; background: var(--rail); padding: 12px 0; border-right: 1px solid var(--border); }
                  .server-rail { display: flex; flex-direction: column; align-items: center; gap: 10px; }
                  .server-item { position: relative; width: 48px; height: 48px; border-radius: 50%; overflow: visible; border: none; padding: 0; background: var(--panel); color: #fff; font-weight: 700; display: grid; place-items: center; transition: border-radius .15s ease, background .15s ease, transform .15s ease; }
                  .server-item:hover { border-radius: 16px; background: #404249; }
                  .server-item.active { border-radius: 16px; background: var(--primary); transform: scale(1.1); }
                  .server-item-indicator { position: absolute; left: -9px; width: 4px; height: 20px; border-radius: 999px; background: #fff; opacity: 0; transform: scaleY(0.5); transition: opacity .15s ease, transform .15s ease, height .15s ease; }
                  .server-item.active .server-item-indicator { opacity: 1; transform: scaleY(1); height: 28px; }
                  .server-avatar { width: 100%; height: 100%; object-fit: cover; object-position: center; display: block; border-radius: inherit; }
                  .server-fallback { font-weight: 700; font-size: 0.8rem; }
                  .main-tabs { display: flex; gap: 8px; margin-bottom: 14px; }
                  .main-tab.active { background: var(--primary); border-color: transparent; }
                  .server-status { position: absolute; right: -3px; bottom: -3px; width: 12px; height: 12px; border-radius: 999px; border: 2px solid var(--rail); background: #3ba55d; z-index: 2; }
                  .server-status.offline { background: #747f8d; }
                  .content { flex: 1; background: var(--content-bg); min-width: 0; }
                  .topbar { display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; padding: 14px 20px; border-bottom: 1px solid var(--border); }
                  .brand { font-size: 1rem; font-weight: 700; }
                  .center-title { text-align: center; font-weight: 700; font-size: 1rem; }
                  ...
                  .pill { padding: 4px 9px; border-radius: 999px; font-size: 0.76rem; border: 1px solid var(--border); color: var(--muted); }
                  button { border: 1px solid var(--border); background: var(--panel-2); color: var(--text); border-radius: 8px; padding: 8px 12px; }
                  ...
                  .home-categories { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 12px; }
                  .home-category-btn.active { background: var(--primary); border-color: transparent; }
                  .home-sections { display: flex; flex-wrap: wrap; gap: 12px; margin-bottom: 12px; }
                  .home-section-panel { flex: 0 0 100%; max-width: 100%; }
                  .home-width-50 { flex-basis: calc(50% - 6px); max-width: calc(50% - 6px); }
                  .home-width-33 { flex-basis: calc(33.333333% - 8px); max-width: calc(33.333333% - 8px); }
                  .home-width-20 { flex-basis: calc(20% - 9.6px); max-width: calc(20% - 9.6px); }
                  @media (max-width: 980px) { .home-width-50, .home-width-33, .home-width-20 { flex-basis: 100%; max-width: 100%; } }
                  ...
                  .home-input, .home-textarea, .home-select { width: 100%; border: 1px solid var(--border); background: var(--panel-2); color: var(--text); border-radius: 8px; padding: 8px 10px; }
                  .home-textarea { min-height: 92px; resize: vertical; }
                  .home-checkbox { width: 18px; height: 18px; }
                  .home-field-row { display: flex; align-items: center; gap: 8px; }
                  .home-message { margin-top: 8px; color: var(--muted); font-size: 0.84rem; }
                  .lookup-results { position: absolute; left: 0; right: 0; top: calc(100% + 6px); z-index: 20; border: 1px solid var(--border); background: var(--panel); border-radius: 8px; max-height: 220px; overflow: auto; display: none; }
                  .lookup-item { width: 100%; border: none; border-radius: 0; text-align: left; padding: 8px 10px; background: transparent; }
                  .lookup-item:hover { background: var(--panel-2); }
                  .lookup-selected { margin-top: 6px; font-size: 0.82rem; color: var(--muted); }
                  .kv { display: grid; gap: 8px; margin-top: 10px; }
                  .kv-item { display: flex; justify-content: space-between; border: 1px solid var(--border); border-radius: 8px; padding: 8px 10px; background: var(--panel-2); }
                  .plugin-fields { display: grid; gap: 10px; margin-top: 10px; }
                  .plugin-field { display: grid; gap: 6px; }
                  .plugin-field > label { color: var(--muted); font-size: 0.84rem; }
                  ...
                  .drag-handle { color: var(--muted); user-select: none; font-size: 0.9rem; }
